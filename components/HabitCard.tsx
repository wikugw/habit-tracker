...
export default function HabitCard({ habit, logs, onLogsChanged, onArchived, onUpdated }: Props) {
  ...
  // Track which day was just toggled so we can animate it
  ...
  const handleToggleToday = useCallback(async () => {
    ...
    if (checkBtnRef.current) {
      checkBtnRef.current.classList.remove('animate-bounce-check')
      void checkBtnRef.current.offsetWidth // reflow to restart
      checkBtnRef.current.classList.add('animate-bounce-check')
    }
    setLoading(true)
    try {
  ...
  return (
    <>
      ...
      <div className="card-enter bg-white dark:bg-zinc-800 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-700 overflow-visible transition-shadow duration-200 hover:shadow-md">
        {/* Header */}
        <div className="flex items-center gap-3 p-4">
          {/* Icon — spins once on expand */}
          <div
            className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl flex-shrink-0 transition-transform duration-300 ${expanded ? 'scale-110' : 'scale-100'}`}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {streak > 0 && (
                <span className="text-xs font-semibold flex items-center gap-0.5 animate-slide-up-sm" style={{ color: habit.color }}>
                  🔥 {streak}d
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-2">
            {/* Check today — bounces on press */}
            <button
              ref={checkBtnRef}
              onClick={handleToggleToday}
              disabled={loading || clearing}
              className={`w-9 h-9 rounded-full flex items-center justify-center transition-colors duration-200 disabled:opacity-60 ${
                isDoneToday
            ...
            <button
              ...
              className="w-9 h-9 rounded-full bg-zinc-100 dark:bg-zinc-700 flex items-center justify-center text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors duration-150 active:scale-90"
            >
              <svg
            ...
            {/* Three-dot menu — menu fades + scales in */}
            <div className="relative">
              ...
              {showMenu && (
                <div className="absolute right-0 top-10 z-50 animate-scale-in-sm bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-zinc-100 dark:border-zinc-700 py-1 min-w-[160px]">
                  <button
                    onClick={() => { setShowMenu(false); setShowEdit(true) }}
                    className="w-full text-left px-4 py-2.5 text-sm text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-700 flex items-center gap-2 transition-colors duration-100"
                  >
                    ✏️ Edit
                  </button>
                  <button
                    onClick={() => { setShowMenu(false); handleClearRecords() }}
                    className="w-full text-left px-4 py-2.5 text-sm text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30 flex items-center gap-2 transition-colors duration-100"
                  >
                    🗑️ Clear Records
                  </button>
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 flex items-center gap-2 transition-colors duration-100"
                  >
                    🗃️ Archive
                  </button>
                </div>
        ...
        {/* Expanded section — slides down */}
        {expanded && (
          <div className="animate-expand-down px-4 pb-4 border-t border-zinc-100 dark:border-zinc-700 pt-3 flex flex-col gap-4">
            {habit.description && (
              <p className="text-sm text-zinc-500 dark:text-zinc-400 animate-fade-in">{habit.description}</p>
            )}
            ...
            <div>
              ...
              <div className="flex gap-1.5 justify-between">
                {last7.map((date, i) => {
                  ...
                  const isJustToggled = justToggled === date
                  return (
                    <button
                      key={date}
                      onClick={() => handleToggleDay(date)}
                      disabled={loading || clearing}
                      className={`flex-1 flex flex-col items-center py-2 rounded-xl transition-colors duration-150 disabled:opacity-60
                        ${isJustToggled ? 'animate-pop' : ''}
            ...
            <div>
              ...
              {clearing ? (
                <div className="flex items-center gap-2 text-sm text-zinc-400 py-2 animate-fade-in">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div
                  key={label}
                  className="rounded-xl py-2 px-3 animate-slide-up-sm"
                  style={{ backgroundColor: habit.color + '18', animationDelay: `${i * 50}ms` }}
                >
